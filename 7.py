...
#            blink = int(now * 2) % 2
        ...
        elif state == STATE_SHUTDOWN_CONFIRM:
            screen.fill(BLACK)

            # Заголовок
            title = FONT_WAIT.render("ВЫКЛЮЧИТЬ?", True, WHITE)
            ...
            pygame.draw.rect(screen, WHITE, no_rect, border_radius=12)

            yes_txt = FONT_MED.render("ДА", True, BLACK)
            no_txt = FONT_MED.render("НЕТ", True, BLACK)
            ...
            # ---- обработка касания ----
            if touch_down:
                px = int(touch_x * WIDTH)
                py = int(touch_y * HEIGHT)

                if yes_rect.collidepoint(px, py):
                    shutdown_raspberry()

                if no_rect.collidepoint(px, py):
                    state = "WAIT_TOUCH"
                    time.sleep(0.2)
